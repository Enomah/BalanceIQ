...
interface SpendingTrendChartProps {
  monthlySummary: { expenses: number; income: number };
}
...
const SpendingTrendChart: React.FC<SpendingTrendChartProps> = ({
  ...
  const weeklyTrendData = [
    {
      week: "Week 1",
      spending: monthlySummary.expenses * 0.2,
      budget: monthlySummary.income * 0.25,
    },
    {
      week: "Week 2",
      spending: monthlySummary.expenses * 0.4,
      budget: monthlySummary.income * 0.25,
    },
    {
      week: "Week 3",
      spending: monthlySummary.expenses * 0.7,
      budget: monthlySummary.income * 0.25,
    },
    {
      week: "Week 4",
      spending: monthlySummary.expenses,
      budget: monthlySummary.income * 0.25,
    },
  ];
  ...
  const CustomTooltip = ({
    ...
    active?: boolean;
    payload?: Array<{ value: number; payload: { week: string } }>;
  }) => {
      ...
      return (
        <div className="bg-[var(--bg-secondary)] p-3 rounded-lg shadow-md border border-[var(--border-light)]">
          ...
          <p className="text-[var(--primary-500)]">
            Weekly Budget:{" "}
            <span className="font-semibold">
              {userProfile?.currency} {payload[1].value.toLocaleString()}
            </span>
          </p>
        </div>
      );
