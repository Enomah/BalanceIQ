...
interface QuickStatsProps {
  monthlySummary: { income: number; expenses: number; balance: number };
}
...
const QuickStats: React.FC<QuickStatsProps> = ({ monthlySummary }) => {
  ...
  const projectedMonthly =
    monthlySummary.expenses * (30 / new Date().getDate());
  const balanceRate =
  ...
  return (
    <div className="grid grid-cols-2 gap-3 mb-4">
      {stats.map((stat, index) => (
        <div
          key={index}
          className="bg-[var(--bg-tertiary)] p-3 rounded-lg text-center"
        >
          <stat.icon
            size={16}
            className="mx-auto mb-1 text-[var(--primary-500)]"
          />
