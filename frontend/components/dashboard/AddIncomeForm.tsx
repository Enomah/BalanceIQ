...
import { Plus, DollarSign, Type, FileText, Loader2 } from "lucide-react";
...
const AddIncomeForm: React.FC<AddIncomeFormProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${baseUrl}/dashboard/incomes`, {
        method: "POST",
        headers: {
      ...
      if (dashboardData) {
        ...
        const newMonthlySummary: MonthlySummary = {
          ...
          income: (dashboardData.monthlySummary.income += transaction.amount),
          balance: (dashboardData.monthlySummary.balance += transaction.amount),
          incomeCategoryTotals: {
            ...dashboardData.monthlySummary.incomeCategoryTotals,
            [transaction.category]: (dashboardData.monthlySummary.incomeCategoryTotals[transaction.category] || 0) + transaction.amount,
          },
        };
